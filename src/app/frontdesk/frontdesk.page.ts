...
buttons: ['OK']
  ...
  async msgcertezacheckin(nome: string, id) {
    const conclui = await this.alertController.create({
      ...
      buttons: [{
        text: "SIM", handler: () => {
          this.httpClient.post(this.instrutor.getUrl() + "/checkin_frontdesk.php", { 'alunoid': id, 'aulaid': this.conteudo.id, 'datahorasolicitacao': this.formatDate(new Date()) }, { responseType: "json", headers: this.headers , observe: "body"})
          .subscribe(
            data => {
              console.log(data)
...
this.msg('Erro',(<erro>error).error.error);
  ...
  pesquisa() {
    this.httpClient.post(this.instrutor.getUrl() + "/aula_filtro.php", { 'search': this.search_string }, { responseType: "json", headers: this.headers })
      .subscribe(
        data => {
          console.log(data);
